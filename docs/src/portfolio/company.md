...
- 반려동물 체온 측정 칩·블루투스 스캐너와 연동하여 체온 데이터를 수집·관리하는 B2C 플랫폼
- NestJS 기반 백엔드 모노레포를 설계하고, 메인 서비스·펫스테이션 등 다수의 도메인 앱과 공유 라이브러리 구조를 구축 (백엔드 전반 담당)
- 개체별 평소체온 산출 알고리즘을 연구·구현하여 개인화된 건강 이상 감지 기능 적용 (특허 작업 포함)
- 쿠폰 시스템, 이벤트 모듈, QR 기반 도장깨기 등 서비스 핵심 기능 개발
- AWS 크레딧 $5,750 확보 및 불필요한 인프라 정리로 클라우드 비용 최적화

### [Service] 와이펫 세이프 — 체온 스캐너 B2B 솔루션
    NestJS(TypeScript), MongoDB, AWS

- 블루투스 체온 스캐너를 활용한 동물 체온 데이터 수집·관리 B2B 솔루션
- B2C 플랫폼 대비 단순화된 구조로, 백엔드 API 설계 및 개발 담당 (프론트엔드·앱은 동료 담당)
- 블루투스 스캐너와의 바이트 레벨 통신 프로토콜 구현 및 앱 연동 모듈 개발

### [System] 펫스테이션 — 반려동물 테마 오프라인 매장 시스템
    NestJS(TypeScript), Next.js, POS(OLLSTAR), 키오스크(KingBrothers), 페이히어, 포스모스

- 반려동물 테마 오프라인 매장(펫스테이션)의 운영 시스템 전체 구축 주도 (2024년 11월 그랜드 오픈)
- POS, 키오스크, 무인주문 시스템 도입 및 다중 결제 수단 연동 (토스 POS, 페이히어, 포스모스)
- 매장 정산 데이터 수집·분석 파이프라인 구축

### [Desktop App] 호원소프트 — 블루투스 스캐너 연동 윈도우 프로그램
    Electron, TypeScript, SQLite, Bluetooth

- 영장류 연구소의 개체 체온 관리를 위한 윈도우 전용 블루투스 연동 프로그램 개발 및 납품
- 블루투스 스캐너 연결/해제, 실시간 데이터 수신 및 바이트 레벨 데이터 해석 구현
- 측정 데이터 SQLite DB 저장 및 클립보드 내보내기 기능 개발
- 기획부터 납품까지 단독 개발 진행 (2024년 12월 1차 납품, 2025년 1월 최종 납품)

### [Leadership] 개발팀 구축 및 운영

- 개발팀 구성부터 기술 방향 설정까지 전반적인 팀 리딩 담당
- 주간 스크럼 도입으로 팀의 업무 예측도 및 진행 속도 향상
- 금요인사이트 운영: 매주 직원 역량 강화 교육 진행 (52회 이상)
- OKR 기반 목표 설정 및 분기별 성과 관리
- AWS, 부산 클라우드 혁신센터, 베스핀글로벌 등 외부 파트너십 구축

### [외주] SB솔루션 — 웹 서비스 유지보수 및 기능 개발
    NestJS(TypeScript), AWS

- 와이펫이 수주한 외주 프로젝트로, 담당 개발자로 참여
- 기존 서비스의 유지보수 및 신규 기능 추가 담당
- 작업 내역을 로그로 관리하며 체계적으로 진행
...
- 유명 쇼핑몰, 포털 사이트 등의 메일 형식을 모방하여 구성한 훈련용 메일로 사용자의 정보 입력 및 첨부파일 다운로드를 유도하고, 그 결과를 보고서로 생성
- 메일 열람 여부, 모방 사이트 접속 여부, 개인정보 입력 여부, 첨부파일 다운로드 여부 등을 추적 및 집계
...
- 기존 시스템의 보안 취약점 및 구조적 문제를 분석하고, 개선된 버전의 개발을 제안하여 주도적으로 진행
- docker-compose.yml 을 이용한 개발 환경 공유
- express 기반의 백엔드 프로젝트와 React 기반의 프론트앤드의 프로젝트를 구분한 구조
...
- 우커머스(쇼핑몰용 위챗 미니 프로그램에서 백엔드로 사용함)에서 발생한 주문 정보를 Zapier와 AWS Lambda 를 이용하여 자동으로 수집함
- 보안 취약점 및 데이터 무결성 이슈를 발견하고 개선
...
- 짧은 기간 내 Java Android 개발을 빠르게 습득하여 기한 내 완료
